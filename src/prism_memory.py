...
def calculate_trust_score(audit, subtext):
    ...
    # Sigmoid logic — k=8 (softer curve than original k=10).
    # k=10 caused a trust collapse of ~38% at D=0.55 (barely above midpoint),
    # which over-penalized borderline dissonance. k=8 keeps the same shape
    # but is more forgiving at the boundary region.
    # If D is 0 (Congruent), Trust is ~0.982
    # If D is 1 (Masked), Trust is ~0.018
    trust = 1 / (1 + math.exp(8 * (D - 0.5)))
    
    # Apply Viscosity Penalty — reduced from ×0.6 to ×0.75.
    # At ×0.6, a D=0.6 + Stuck entry had only 16% influence, effectively
    # silencing the user's correction. "Stuck" states still carry valid
    # emotional signals (ACT principle); ×0.75 discounts without erasing.
...
def save_personality_observation(audit, subtext, raw_text: str = ""):
    ...
    # Personality weight scales with subtext_confidence (0.0–1.0) mapped to
    # a [0.4, 0.8] influence band, replacing the flat ×0.6 multiplier.
    # High-confidence observations (e.g., 0.9) earn ×0.72 influence;
    # low-confidence guesses (e.g., 0.2) earn only ×0.40 — proportional
    # to how certain the model actually was when it made the inference.
    subtext_confidence = getattr(subtext, 'subtext_confidence', 0.5)
    confidence_scale = 0.4 + (subtext_confidence * 0.4)  # Maps [0,1] → [0.4, 0.8]

    observation = {
        "type": "auto_observed",
        "weight": round(trust_score * confidence_scale, 2),  # Scaled by model confidence, not flat multiplier
        "tag": getattr(subtext, 'viscosity_tag', 'Unknown'),
